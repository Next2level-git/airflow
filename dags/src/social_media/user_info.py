...
class User_info:
    ...
    def run(self):
        ...
        if not update_data.empty:
            ...
            self.__logger.info(
                f"The update of {update_data.shape[0]} rows was made."
            )
    ...
    def __transform_data(self, data):
        ...
        if not result_append.empty:
            result_append["publications"] = result_append["publications"].astype("Int64")
            result_append["last_publication"] = result_append["last_publication"].astype("Int64")
            result_append = result_append[
                ['id', 'username', 'full_name',
                 'bio', 'category', 'publications', 'last_publication'
                 ]
            ]
